 ...
 * face/jaw by warping the sampled UVs. Blits the composited image to [target];
...
internal class FaceReshapePass {
    ...
    private val midX = FloatArray(MAX_FACES)
    private val noseC = FloatArray(MAX_FACES * 2)
    private val noseR = FloatArray(MAX_FACES)
    private val jawC = FloatArray(MAX_FACES * 2)
    private val jawR = FloatArray(MAX_FACES)
    ...
    fun draw(
        ...
        if (adjust.reshapes) {
            for (face in faces) {
                ...
                val jcx = (ux(face, FaceTopology.CHEEK_LEFT, viewport) +
                    ux(face, FaceTopology.CHEEK_RIGHT, viewport)) / 2f
                val cheekY = (uy(face, FaceTopology.CHEEK_LEFT, viewport) +
                    uy(face, FaceTopology.CHEEK_RIGHT, viewport)) / 2f
                val chinY = uy(face, FaceTopology.CHIN, viewport)
                jawC[faceCount * 2] = jcx
                jawC[faceCount * 2 + 1] = cheekY + (chinY - cheekY) * 0.45f
                jawR[faceCount] = scale * 1.25f
                faceCount++
        ...
        // Only feed the nose/jaw arrays when those warps are active.
        val faceN = if (adjust.noseSlim > 0f || adjust.faceSlim > 0f) faceCount else 0
        ...
        if (faceN > 0) {
            GLES20.glUniform1fv(loc("uMidX"), faceN, midX, 0)
            GLES20.glUniform2fv(loc("uNoseC"), faceN, noseC, 0)
            GLES20.glUniform1fv(loc("uNoseR"), faceN, noseR, 0)
            GLES20.glUniform2fv(loc("uJawC"), faceN, jawC, 0)
            GLES20.glUniform1fv(loc("uJawR"), faceN, jawR, 0)
        }
